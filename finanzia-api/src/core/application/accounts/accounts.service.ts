...
export class AccountsService {
  ...
  async getUserAccounts(userId: string, includeArchived = false): Promise<AccountResponseDto[]> {
    const accounts = await this.accountRepository.findAllByUserId(userId, includeArchived);
    return accounts.map((account) => this.toResponseDto(account));
  }

  async getAccountById(userId: string, accountId: string): Promise<AccountResponseDto> {
  ...
  async archiveAccount(userId: string, accountId: string): Promise<AccountResponseDto> {
    // Valida existencia y pertenencia al usuario
    await this.getAccountById(userId, accountId);
