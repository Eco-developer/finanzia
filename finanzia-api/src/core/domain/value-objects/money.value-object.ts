...
export class Money {
  ...
  public toCentsNumber(): number {
    if (
      this.amountInCents > BigInt(Number.MAX_SAFE_INTEGER) ||
      this.amountInCents < BigInt(Number.MIN_SAFE_INTEGER)
    ) {
      throw new InvalidMonetaryAmountException(
        "El importe supera el rango entero seguro para conversión a Number",
      );
    }
