...
export class JwtStrategy extends PassportStrategy(Strategy, 'jwt') {
  constructor(
    ...
    super({
      ...
      secretOrKey: configService.get<string>('JWT_SECRET') || 'dev_jwt_secret_finanzia_super_secure_32_chars',
    });
  }
  ...
  async validate(payload: JwtPayload) {
    ...
    if (!user) {
      throw new UnauthorizedException('El token no corresponde a ningún usuario válido');
    }
    return {
      id: user.id,
      email: user.email,
